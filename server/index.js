...
// API Endpoints
app.get('/api/houses/:id', (req, res, next) => {
...
app.get('/api/photos/houses/:id', (req, res, next) => {
...
app.post('/api/photos/houses/:id', (req, res, next) => {
  const houseId = Number(req.params.id)
  const photoUrl = 780
  const photoId = 9000000;

  pool.query(`INSERT INTO photos (photo_id, house_id, photoUrl) VALUES (${photoId}, ${houseId}, ${photoUrl});`, (err, results) => {
    if (err) {
...
app.get('/api/bedrooms/houses/:id', (req, res, next) => {

  var houseId = Number(req.params.id);

  pool.query(`SELECT * FROM bedrooms WHERE house_id = ${houseId};`, (err, results) => {
    if (err) {
      console.log('Error getting house.', err)
      res.status(404).send({success: false, message: 'Could not fetch bedrooms for house from our database.'});
    } else {
      console.log('House results', results.rows)
      res.status(200).send(results.rows);
    }
...
app.post('/api/bedrooms/houses/:id', (req, res, next) => {
  const houseId = Number(req.params.id)
